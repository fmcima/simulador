...
const ComparisonView = ({ projectA, setProjectA, projectB, setProjectB, resultsA, resultsB, copyAToB }) => {
    ...
    const getDiffColor = (valA, valB, type) => {
        if (!valA || !valB) return 'text-slate-400';
        if (valA === valB) return 'text-slate-400';
        const isBetter = type === 'higher' ? valB > valA : valB < valA;
        return isBetter ? 'text-emerald-600 font-bold' : 'text-red-500';
    };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
            ...
            <div className="lg:col-span-3 space-y-4">
                <div className="flex items-center justify-between bg-blue-50 p-3 rounded-lg border border-blue-100">
                    <div className="text-blue-800 text-sm font-bold flex items-center gap-2">
                ...
                <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm h-80">
                    ...
                    <ResponsiveContainer width="100%" height="90%">
                        <LineChart data={comparisonData} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
                            ...
                            <YAxis tickFormatter={(v) => `$${(v / 1000000000).toFixed(1)}B`} fontSize={10} tick={{ fill: '#64748b' }} axisLine={false} tickLine={false} />
                            <Tooltip formatter={(v) => formatCurrency(v)} labelFormatter={(l) => `Ano ${l}`} contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} />
                            ...
                            {metrics.map((m, i) => {
                                ...
                                return (
                                    <tr key={m.key} className="hover:bg-slate-50/50 transition-colors">
                                        <td className="p-3 font-medium text-slate-700">{m.label}</td>
                                        <td className="p-3 font-bold text-blue-700">{m.fmt(valA)}</td>
            ...
            <div className="lg:col-span-3 space-y-4">
                <div className="flex items-center justify-between bg-orange-50 p-3 rounded-lg border border-orange-100">
                    <div className="text-orange-800 text-sm font-bold flex items-center gap-2">
                        <Settings className="w-4 h-4" /> Parâmetros Cenário B
                    </div>
                    <button
                        onClick={copyAToB}
                        className="text-xs bg-white text-orange-600 border border-orange-200 px-3 py-1 rounded hover:bg-orange-100 transition-colors flex items-center gap-1"
